...
## Define the least squares objective
def Eq_obj(beta, Sv_N, SvS, S_v_T, S_f_T, alpha, K, FxdM):
    alpha = np.reshape(alpha,(len(alpha),1))
    y_e = np.matmul(Sv_N,np.reshape(beta,(np.shape(beta)[0],1)   ) )
    #y_hat_e = np.log(K) - np.log(2) - np.log(alpha) + np.log( - y_e + np.sqrt( np.power(y_e,2) + 4*np.power(alpha,2) )  )
    y_hat_e = np.log(K) - np.log(2) - np.log(alpha) + np.log(2*alpha) + (y_e*1e10)/(np.abs(y_e)*1e10 + 1e-12)*( np.log(2*alpha) - np.log( np.abs(y_e) + np.sqrt( np.power(y_e,2) + 4*np.power(alpha,2) ) ))
    b_e = y_hat_e - np.matmul(S_f_T,np.reshape(FxdM,(len(FxdM),1) )    )
    return np.reshape( np.matmul( S_v_T, np.matmul(SvS,b_e) ) - b_e , (len(alpha),) )
...
def Feasible_point_solver(v_log_counts, target_log_vcounts, f_log_counts, S, K):
  ...
  # Flip Stoichiometric Matrix
    ...
    n_M_v = len(v_log_counts)   #total number of variable metabolites
    ...
    b = np.zeros(dSv_N)
    for i in range(dSv_N):
        b[i] = np.sum(Sv_N[181:184,i] )
    ...
    alpha = np.ones(n_react)
    # Get the initial least squares solution without optimizing regulation
    #beta = np.random.uniform(-1,1,(dSv_N,))
    beta = b
    #betlsq = least_squares(Eq_obj,beta,args=(Sv_N,SvS,S_v_T,alpha,K_eq,FxdM),ftol=None,xtol=None,max_nfev=4000)
    betlsq = least_squares(Eq_obj,beta,method='lm',args=(Sv_N,SvS,S_v_T,S_f_T,alpha,K_eq,FxdM),ftol=2.23e-16,xtol=2.23e-16,max_nfev=100000)

    
    ## Extract the solution
    y_sol = np.matmul(Sv_N,np.reshape(betlsq['x'],(dSv_N,1) ) )
    
    h_sol = (y_sol*1e25)*np.power(np.abs(y_sol)*1e25 + 1e-25,-1)*( np.log(2) - np.log( np.abs(y_sol) + np.sqrt(np.power(y_sol,2) + 4  )))   
    
    b_RHS = np.log(K_eq) + h_sol  - np.matmul(S_f_T,np.reshape(FxdM,(n_M_f,1) )    ) 
    n_sol = np.matmul(SvS,b_RHS)
    
    b_sol = np.matmul(S_v_T,np.reshape(n_sol,(len(n_sol),1))) + np.matmul(S_f_T,np.reshape(FxdM,(n_M_f,1) )    )
    b_sol = np.reshape(b_sol,len(b_sol))
    ...
    # Set initial conditions for ipopt
    n_ini = np.reshape(n_sol,(len(n_sol),) )
    b_ini = np.reshape(b_sol,(len(b_sol),) )
    y_ini = np.reshape(y_sol,(len(y_sol),) )
    beta_ini = np.reshape(beta_sol,(len(beta_sol),) )
    h_ini = np.reshape(h_sol,(len(h_sol),) )
    

    ## Set the optimization parameters
    VarM_lbnd = -300 #lower bound on the log metabolite counts
    #alpha_lb = 1e-5 #lower bound for the reaction regulation
    ...
    def _Obj(m):
        return sum (   (m.b[j]  - sum( m.S[(k,j)]*m.VarM[k] for k in m.VarM_idx ) - sum( m.S[(k,j)]*m.FxdM[k] for k in m.FxdM_idx ) )**2   for j in m.react_idx)
    m.Obj_fn = pe.Objective(rule = _Obj, sense = pe.minimize)
...
#     b_sol = np.reshape(b_sol, (len(b_sol)) )
#     y_sol = np.reshape(y_sol, (len(y_sol)) )
#     alpha_sol = np.reshape(lalpha_sol, (len(lalpha_sol)) )
#     h_sol = np.reshape(h_sol, (len(h_sol)) )
#     beta_sol = np.reshape(beta_sol, (len(beta_sol)) )
#     n_sol = np.reshape(n_sol, (len(n_sol)) )
    
#     solved =1
#     feas_obj=1e-18

    return(solved,feas_obj,b_sol, y_sol, alpha_sol, h_sol, beta_sol, n_sol)
...
def Eq_lobj(beta, Sv_N, SvS, S_v_T, S_f_T, lalpha, K, FxdM):
    lalpha = np.reshape(lalpha,(len(lalpha),1))
    y_e = np.matmul(Sv_N,np.reshape(beta,(np.shape(beta)[0],1)   ) )
    #y_hat_e = np.log(K) - np.log(2) - np.log(alpha) + np.log( - y_e + np.sqrt( np.power(y_e,2) + 4*np.power(alpha,2) )  )
    y_hat_e = np.log(K) + (y_e*1e10)/(np.abs(y_e)*1e10 + 1e-12)*( np.log(2) + lalpha - np.log( np.abs(y_e) + np.sqrt( np.power(y_e,2) + 4*np.exp(2*lalpha) ) ))
    b_e = y_hat_e - np.matmul(S_f_T,np.reshape(FxdM,(len(FxdM),1) )    )
    return np.reshape( np.matmul( S_v_T, np.matmul(SvS,b_e) ) - b_e , (len(lalpha),) )
...
def Random_Feasible_Point_solver(v_log_counts, target_log_vcounts, f_log_counts, S, K):
...
#     # Generate a random initial flux
    
#     bmax = 100
#     bmin = -100
    
#     beta = np.random.uniform(0,1,dSv_N)
#     beta=bmin + beta*(bmax-bmin)
    
#     beta = np.reshape(beta,(len(beta),))
    ...
    #y_sol = np.matmul(Sv_N,np.reshape(beta,(dSv_N,1) ) )
    ...
    def y_sign_relax(m,i):
        return 2*m.u[i] - 1 == (m.y[i]/(abs(m.y[i]) + 1e-25) ) 
    m.y_sign_relax_cns = pe.Constraint(m.react_idx,rule = y_sign_relax)
